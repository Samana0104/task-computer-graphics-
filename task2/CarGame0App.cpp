...
float g_valueForbirdEye = 0.f;
float g_valueForbirdEyeSpeed = 0.f;
...
void Display()
{
	...
	float fov1 = glm::radians<float>(35.f);
	float fov2 = glm::radians<float>(85.f);

	glm::vec3 viewForFront = glm::rotateY(glm::vec3(0.f, 0.f, 0.1f), g_car_rotation_y);
	glm::vec3 viewForTop(0.f, 0.1f, 0.f);
	glm::vec3 viewForSide = glm::rotateY(glm::vec3(0.1f, 0.0f, 0.f), g_car_rotation_y);

	glm::vec3 viewOfEye1 = g_car_poisition + viewForFront * -40 + viewForTop * 18;
	glm::vec3 viewOfEye2 = g_car_poisition + viewForSide * 1.2 + (-1.2 * viewForFront) + viewForTop * 6;
	glm::vec3 viewOfCenter1 = g_car_poisition + viewForFront * 15 + viewForTop * 6;
	glm::vec3 viewOfCenter2 = g_car_poisition + viewForSide * 1.2 + viewForFront * 15 + viewForTop * 6;

	projection_matrix = glm::perspective(fov1 + glm::abs((fov2 - fov1) * g_valueForbirdEye), (float)g_window_w / g_window_h, 0.01f, 10000.f);

	if (g_camera_mode == 0)
	{
		projection_matrix = glm::perspective(fov2, (float)g_window_w / g_window_h, 0.01f, 10000.f);
		view_matrix = g_camera.GetGLViewMatrix();
	}
	else
	{
		view_matrix = glm::lookAt(viewOfEye1 + (viewOfEye2 - viewOfEye1) * g_valueForbirdEye,
			viewOfCenter1 + (viewOfCenter2 - viewOfCenter1) * g_valueForbirdEye,
...
void Timer(int value)
{
	...
	g_valueForbirdEye += g_valueForbirdEyeSpeed;

	if (g_valueForbirdEye <= 0)
	{
		g_valueForbirdEyeSpeed = 0.0f;
		g_valueForbirdEye = 0.0f;
	} 
	else if (g_valueForbirdEye >= 1)
	{
		g_valueForbirdEyeSpeed = 0.0f;
		g_valueForbirdEye = 1.0f;
	}
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	{
	case 's':
		g_car_speed = -0.04f;		// 후진 속도 설정
		glutPostRedisplay();
		break;

	case 'w':
		g_car_speed = 0.04f;		// 전진 속도 설정
		glutPostRedisplay();
		break;

	case 'a':
		if (g_car_speed >= 0.f)
			g_car_angular_speed = glm::radians(1.5f);		// 좌회전 각속도 설정
		else
			g_car_angular_speed = glm::radians(-1.5f);
		glutPostRedisplay();
		break;

	case 'd':
		if (g_car_speed >= 0.f)
			g_car_angular_speed = -1 * glm::radians(1.5f);		// 우회전 각속도 설정
		else
			g_car_angular_speed = -1 * glm::radians(-1.5f);
		glutPostRedisplay();
		break;
	...
	case '2':
		if (g_camera_mode == 0)
		{
			g_valueForbirdEyeSpeed = 0.f;
			g_valueForbirdEye = 0.f;
		}
		else
		{
			g_valueForbirdEyeSpeed = -0.01f;
		}
		g_camera_mode = 1;
...
void Mouse(int button, int state, int x, int y)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && g_camera_mode == 0)
		g_left_button_pushed = true;

	else if (button == GLUT_LEFT_BUTTON && state == GLUT_UP)
		g_left_button_pushed = false;

	else if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN && g_camera_mode == 0)
